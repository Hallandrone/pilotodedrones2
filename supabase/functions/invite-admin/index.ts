import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

// Environment variables
const RESEND_API_KEY = Deno.env.get('RESEND_API_KEY')!
const SUPABASE_URL = Deno.env.get('SUPABASE_URL')!
const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
const SUPABASE_ANON_KEY = Deno.env.get('SUPABASE_ANON_KEY')!
const FRONTEND_URL = Deno.env.get('FRONTEND_URL') || 'https://pilotodedrones.cl'

const corsHeaders = {
	'Access-Control-Allow-Origin': '*',
	'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
}

function generatePassword(length = 8) {
	const charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
	let retVal = "";
	for (let i = 0, n = charset.length; i < length; ++i) {
		retVal += charset.charAt(Math.floor(Math.random() * n));
	}
	return retVal;
}

const PERMISSION_LABELS: Record<string, string> = {
	create_diplomas: 'Crear Diplomas',
	manage_certificates: 'Gestionar Certificados',
	approve_deny_certificates: 'Aprobar/Denegar Certificados',
	view_users: 'Ver Usuarios',
	view_companies: 'Ver Empresas',
	view_notifications: 'Ver Notificaciones',
	manage_banners: 'Gestionar Banners',
	upload_banners: 'Subir Banners de Publicidad',
};

serve(async (req) => {
	if (req.method === 'OPTIONS') {
		return new Response('ok', { headers: corsHeaders })
	}

	try {
		// 1. Security Check
		const authHeader = req.headers.get('Authorization')
		if (!authHeader) {
			return new Response(JSON.stringify({ error: 'No autorizado' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
		}

		const supabaseClient = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { global: { headers: { Authorization: authHeader } } })
		const { data: { user: currentUser }, error: authError } = await supabaseClient.auth.getUser()

		if (authError || !currentUser) {
			return new Response(JSON.stringify({ error: 'Token inválido o expirado' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
		}

		const supabaseAdmin = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)

		// Verify Caller Role (Must be Admin or Super Admin)
		const { data: roleData } = await supabaseAdmin
			.from('user_roles')
			.select('role')
			.eq('id', currentUser.id)
			.single()

		const isAllowed = roleData?.role === 'super_admin' || roleData?.role === 'admin';
		if (!isAllowed) {
			return new Response(JSON.stringify({ error: 'Permisos insuficientes.' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
		}

		// 2. Process Request
		const { email, full_name, permissions, role, password } = await req.json()

		if (!email) {
			return new Response(JSON.stringify({ error: 'Email es requerido' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
		}

		const cleanEmail = email.toLowerCase().trim()
		const targetRole = role || 'pilot'; // Default a pilot si no se especifica
		const invitedBy = currentUser.id

		// 3. Find user by email in Auth - More reliable lookup
		console.log('Searching for user:', cleanEmail);
		const { data: listData, error: listError } = await supabaseAdmin.auth.admin.listUsers();
		if (listError) {
			console.error('Error listing users:', listError);
			throw listError;
		}

		const targetUser = listData.users.find(u => u.email?.toLowerCase() === cleanEmail);
		let targetUserId = targetUser?.id;
		let autoGeneratedPassword = '';
		let isNewUser = false;

		if (!targetUserId) {
			// --- CREATE NEW USER ---
			isNewUser = true;
			console.log('User not found. Creating new user...');
			// Usar contraseña proporcionada o generar una
			autoGeneratedPassword = password || generatePassword(8);

			const { data: newUser, error: createError } = await supabaseAdmin.auth.admin.createUser({
				email: cleanEmail,
				password: autoGeneratedPassword,
				email_confirm: true, // Auto confirm so they can login immediately
				user_metadata: { full_name: full_name || '' }
			});

			if (createError) {
				console.error('Error creating user:', createError);
				throw createError;
			}

			targetUserId = newUser.user.id;

			// Create profile for new user
			const { error: profileError } = await supabaseAdmin.from('profiles').upsert({
				id: targetUserId,
				email: cleanEmail,
				full_name: full_name || '',
				user_type: targetRole
			});

			if (profileError) console.error('Error creating profile:', profileError);
			console.log('New user created with password:', autoGeneratedPassword);
		} else {
			console.log('Existing user found with ID:', targetUserId);
			// Update profile user_type if it's an existing user to match new role
			const { error: profileUpdateError } = await supabaseAdmin
				.from('profiles')
				.update({ user_type: targetRole, full_name: full_name || '' })
				.eq('id', targetUserId);

			if (profileUpdateError) console.error('Error updating profile:', profileUpdateError);
		}

		// --- ASSIGN ROLE ---
		const { data: existingRole, error: roleFetchError } = await supabaseAdmin
			.from('user_roles')
			.select('role')
			.eq('id', targetUserId)
			.maybeSingle();

		if (roleFetchError) console.error('Error fetching existing role:', roleFetchError);

		// Solo actualizar el rol si es diferente o no existe
		// No downgrade de super_admin
		if (existingRole?.role !== 'super_admin') {
			console.log(`Setting role to ${targetRole} for user ${targetUserId}`);
			const { error: roleUpdateError } = await supabaseAdmin.from('user_roles').upsert({
				id: targetUserId,
				role: targetRole as 'pilot' | 'company' | 'admin' | 'super_admin'
			});
			if (roleUpdateError) console.error('Error updating role:', roleUpdateError);
		} else {
			console.log('User is already super_admin, not changing role.');
		}

		// --- ASSIGN PERMISSIONS (solo para admin/super_admin) ---
		if ((targetRole === 'admin' || targetRole === 'super_admin') && permissions) {
			console.log('Updating permissions for admin user...');
			// Update Permissions
			const { error: deleteError } = await supabaseAdmin.from('user_permissions').delete().eq('user_id', targetUserId);
			if (deleteError) console.error('Error deleting old permissions:', deleteError);

			if (permissions.length > 0) {
				const permsToInsert = permissions.map((p: string) => ({
					user_id: targetUserId,
					permission: p,
					granted_by: invitedBy
				}));
				const { error: insertError } = await supabaseAdmin.from('user_permissions').insert(permsToInsert);
				if (insertError) console.error('Error inserting new permissions:', insertError);
			}
		}

		// --- PREPARE EMAILS (para admin/super_admin siempre) ---
		let emailSubject = '';
		let emailHtml = '';
		const mappedPermissions = permissions ? permissions.map((p: string) => PERMISSION_LABELS[p] || p) : [];

		const shouldSendEmail = (targetRole === 'admin' || targetRole === 'super_admin');

		console.log('Email sending logic check:', {
			targetRole,
			shouldSendEmail,
			isNewUser,
			hasResendKey: !!RESEND_API_KEY,
			email: cleanEmail
		});

		if (shouldSendEmail) {
			if (isNewUser) {
				// Email para usuario NUEVO con credenciales
				emailSubject = '¡Bienvenido! Tu cuenta de Administrador ha sido creada';
				emailHtml = `
      <!DOCTYPE html>
      <html>
      <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
        <div style="background-color: #083b4e; padding: 25px; text-align: center;">
          <h1 style="color: white; margin: 0;">Piloto de Drones</h1>
        </div>
        <div style="padding: 30px; border: 1px solid #edf2f7; border-top: 0; border-radius: 0 0 8px 8px;">
          <h2 style="color: #2d3748;">${full_name ? `Hola ${full_name},` : '¡Hola!'}</h2>
          <p>Has sido invitado a formar parte del equipo de <strong>Administradores</strong> en Piloto de Drones.</p>
          
          <div style="background: #f8fafc; padding: 20px; border-radius: 12px; margin: 25px 0; border: 1px solid #e2e8f0;">
            <p style="margin: 0 0 10px; color: #4a5568;">Tu cuenta ya ha sido creada. <strong>No necesitas registrarte</strong>, solo ingresa con estos datos:</p>
            <p style="margin: 5px 0;"><strong>Email:</strong> ${cleanEmail}</p>
            <p style="margin: 5px 0;"><strong>Contraseña Temporal:</strong> <span style="background: #edf2f7; padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 1.1em;">${autoGeneratedPassword}</span></p>
          </div>

          <p>Importante: Ve directamente a la pestaña de <strong>"Iniciar Sesión"</strong>. No intentes crear una cuenta de nuevo ya que tu email ya está registrado.</p>
          
          <div style="margin: 30px 0; text-align: center;">
            <a href="${FRONTEND_URL}/auth" style="background-color: #00b3f3; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 4px 6px rgba(0,179,243,0.2);">Ir a Iniciar Sesión</a>
          </div>

          <div style="border-top: 1px solid #edf2f7; padding-top: 20px; margin-top: 30px;">
            <p style="font-size: 0.9em; color: #718096; margin-bottom: 10px;">Accesos habilitados en tu cuenta:</p>
            <ul style="color: #4a5568; font-size: 0.9em; margin: 0; padding-left: 20px;">
              ${mappedPermissions.map((label: string) => `<li>${label}</li>`).join('')}
            </ul>
          </div>
        </div>
      </body>
      </html>
      `;
			} else {
				// Email para usuario EXISTENTE - notificación de permisos actualizados
				emailSubject = 'Tus permisos de Administrador han sido actualizados';
				emailHtml = `
      <!DOCTYPE html>
      <html>
      <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
        <div style="background-color: #083b4e; padding: 25px; text-align: center;">
          <h1 style="color: white; margin: 0;">Piloto de Drones</h1>
        </div>
        <div style="padding: 30px; border: 1px solid #edf2f7; border-top: 0; border-radius: 0 0 8px 8px;">
          <h2 style="color: #2d3748;">${full_name ? `Hola ${full_name},` : 'Hola,'}</h2>
          <p>Te informamos que tus privilegios de <strong>Administrador</strong> en Piloto de Drones han sido otorgados o actualizados.</p>
          
          <div style="background: #e6f7ff; padding: 20px; border-radius: 12px; margin: 25px 0; border-left: 4px solid #00b3f3;">
            <p style="margin: 0 0 10px; font-weight: bold;">Tus permisos actuales:</p>
            <ul style="margin: 0; padding-left: 20px;">
              ${mappedPermissions.map((label: string) => `<li>${label}</li>`).join('')}
            </ul>
          </div>

          <p>Puedes acceder a estas herramientas iniciando sesión normalmente en la plataforma profesional con tu contraseña habitual.</p>
          
          <div style="margin: 30px 0; text-align: center;">
            <a href="${FRONTEND_URL}/dashboard" style="background-color: #00b3f3; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: bold; display: inline-block; box-shadow: 0 4px 6px rgba(0,179,243,0.2);">Ir al Panel de Control</a>
          </div>
        </div>
      </body>
      </html>
      `;
			}

			console.log(`Attempting to send email via Resend to ${cleanEmail}`);

			// --- SEND EMAIL ---
			if (RESEND_API_KEY) {
				try {
					const emailRes = await fetch('https://api.resend.com/emails', {
						method: 'POST',
						headers: {
							'Authorization': `Bearer ${RESEND_API_KEY}`,
							'Content-Type': 'application/json'
						},
						body: JSON.stringify({
							from: 'Piloto de Drones <info@pilotodedrones.cl>',
							to: cleanEmail,
							subject: emailSubject,
							html: emailHtml
						})
					});

					const emailResponseData = await emailRes.json();

					if (!emailRes.ok) {
						console.error('RESEND ERROR:', emailResponseData);
					} else {
						console.log('Email sent successfully via Resend:', emailResponseData.id);
					}
				} catch (emailError: any) {
					console.error('Exception while sending email:', emailError.message);
				}
			} else {
				console.error('⚠️ RESEND_API_KEY NOT FOUND IN ENVIRONMENT');
			}
		}

		// Preparar mensaje de respuesta
		let message = '';
		if (isNewUser) {
			if (targetRole === 'admin' || targetRole === 'super_admin') {
				message = `Usuario creado con éxito y credenciales enviadas a ${cleanEmail}`;
			} else {
				message = `Usuario ${cleanEmail} creado exitosamente como ${targetRole}`;
			}
		} else {
			if (targetRole === 'admin' || targetRole === 'super_admin') {
				message = `Accesos actualizados y email enviado a ${cleanEmail}`;
			} else {
				message = `Usuario ${cleanEmail} actualizado a rol ${targetRole}`;
			}
		}

		return new Response(
			JSON.stringify({ success: true, message }),
			{ status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
		)

	} catch (error: any) {
		console.error('Function error:', error)
		return new Response(
			JSON.stringify({ error: error.message || 'Error interno del servidor' }),
			{ status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
		)
	}
})

